...
open class SpriteElementScene: SKScene, SKPhysicsContactDelegate {
    ...
    typealias SpriteElementEnumerator = (_ element: SpriteElement, _ node: SKNode) -> Void
    ...
    private var attachedElements: WeakKeyDictionary<SKNode, SpriteElementCollectionBox>
        = WeakKeyDictionary<SKNode, SpriteElementCollectionBox>(valuesRetainedByKey: true)
    ...
    private func _enumerateSpriteElements(_ callback: SpriteElementEnumerator) {
    ...
    func testContact(_ body: SKPhysicsBody, _ callback: SpriteElementEnumerator) {
    ...
    open func didBegin(_ contact: SKPhysicsContact) {
        testContact(contact.bodyA) { element, node in
            element.didBegin(contact: contact, node: node)
        }

        testContact(contact.bodyB) { element, node in
            element.didBegin(contact: contact, node: node)
        }
    }
...
class SpriteElementCollectionBox {
    ...
    func detach(_ element: SpriteElement) {
        elements = elements.filter({ (element: SpriteElement) -> Bool in
